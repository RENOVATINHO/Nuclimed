...
const DADOS_LINHA = [
  { semana: "03/02", receitas: 8400, despesas: 4200 },
  { semana: "10/02", receitas: 12800, despesas: 5100 },
  { semana: "17/02", receitas: 9600, despesas: 6200 },
  { semana: "24/02", receitas: 15200, despesas: 4800 },
  { semana: "03/03", receitas: 11400, despesas: 5600 },
  { semana: "10/03", receitas: 18600, despesas: 7200 },
];

const CATEGORIAS_SERVICO = [
  { nome: "Consultas", valor: 48200, cor: "#6D28D9" },
  { nome: "Procedimentos", valor: 22800, cor: "#0EA5E9" },
  { nome: "Cirurgias", valor: 18600, cor: "#F59E0B" },
  { nome: "Exames", valor: 12400, cor: "#10B981" },
  { nome: "Retornos", valor: 7600, cor: "#EC4899" },
];

const ULTIMAS_TRANSACOES: Transacao[] = [
  { id: "1", data: "12/03/2026", paciente: "Fernanda Lima", descricao: "Consulta clínica", categoria: "Consultas", convenio: "Particular", valor: 200, status: "pago", tipo: "receita" },
  { id: "2", data: "12/03/2026", paciente: "Gabriel Torres", descricao: "Retorno", categoria: "Retornos", convenio: "Unimed", valor: 80, status: "pendente", tipo: "receita" },
  { id: "3", data: "12/03/2026", paciente: "—", descricao: "Material cirúrgico", categoria: "Material", valor: 320, status: "pago", tipo: "despesa" },
  { id: "4", data: "11/03/2026", paciente: "Pedro Henrique", descricao: "Consulta clínica", categoria: "Consultas", convenio: "SulAmérica", valor: 180, status: "pago", tipo: "receita" },
  { id: "5", data: "11/03/2026", paciente: "Juliana Mendes", descricao: "Procedimento ambulatorial", categoria: "Procedimentos", convenio: "Unimed", valor: 350, status: "pago", tipo: "receita" },
  { id: "6", data: "10/03/2026", paciente: "—", descricao: "Aluguel clínica", categoria: "Aluguel", valor: 3200, status: "pago", tipo: "despesa" },
  { id: "7", data: "10/03/2026", paciente: "Mariana Costa", descricao: "Consulta clínica", categoria: "Consultas", convenio: "Particular", valor: 250, status: "pago", tipo: "receita" },
  { id: "8", data: "09/03/2026", paciente: "—", descricao: "Salário secretária", categoria: "Salários", valor: 1800, status: "pago", tipo: "despesa" },
  { id: "9", data: "09/03/2026", paciente: "Ana Paula", descricao: "Retorno", categoria: "Retornos", convenio: "Bradesco Saúde", valor: 80, status: "pago", tipo: "receita" },
  { id: "10", data: "09/03/2026", paciente: "Carlos Eduardo", descricao: "Consulta clínica", categoria: "Consultas", convenio: "Amil", valor: 200, status: "cancelado", tipo: "receita" },
];

const TOTAL_RECEITAS = 76600;
const TOTAL_DESPESAS = 31200;
...
const PIE_TOTAL = CATEGORIAS_SERVICO.reduce((s, c) => s + c.valor, 0);

function CustomPieTooltip({ active, payload }: { active?: boolean; payload?: PiePayload[] }) {
  if (!active || !payload?.length) return null;
  return (
    <div className="bg-white border rounded-xl shadow-lg p-3 text-xs">
      ...
      <p className="text-slate-400">{((payload[0].value / PIE_TOTAL) * 100).toFixed(1)}%</p>
    </div>
  );
}
        ...
        <div className="bg-white rounded-xl border shadow-sm p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-3xl font-bold text-emerald-600 mt-1 tabular-nums">{moeda(TOTAL_RECEITAS)}</p>
            </div>
        ...
        <div className="bg-white rounded-xl border shadow-sm p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-3xl font-bold text-red-500 mt-1 tabular-nums">{moeda(TOTAL_DESPESAS)}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border shadow-sm p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={DADOS_LINHA} margin={{ top: 4, right: 8, bottom: 0, left: -10 }}>
        ...
        <div className="bg-white rounded-xl border shadow-sm p-5 flex flex-col">
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={CATEGORIAS_SERVICO} cx="50%" cy="50%" innerRadius={46} outerRadius={70} paddingAngle={3} dataKey="valor" nameKey="nome">
                  {CATEGORIAS_SERVICO.map((entry, idx) => (
            ...
            <div className="w-full space-y-2 mt-1">
              {CATEGORIAS_SERVICO.map((c) => (
                <div key={c.nome} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="font-semibold text-slate-700">{((c.valor / PIE_TOTAL) * 100).toFixed(0)}%</span>
      ...
      <div className="bg-white rounded-xl border shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {ULTIMAS_TRANSACOES.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50/40 transition-colors">
                  <td className="px-4 py-3 text-slate-400 text-xs whitespace-nowrap">{t.data}</td>
                  <td className="px-4 py-3 text-slate-700 font-medium text-xs">{t.paciente}</td>
                  <td className="px-4 py-3 text-slate-600 text-xs">{t.descricao}</td>
                  <td className="px-4 py-3 text-slate-500 text-xs">{t.categoria}</td>
                  <td className="px-4 py-3 text-slate-400 text-xs">{t.convenio || "—"}</td>
                  <td className="px-4 py-3 text-right">
                    <span className={cn("text-sm font-bold tabular-nums", t.tipo === "receita" ? "text-emerald-600" : "text-red-500")}>
                      {t.tipo === "despesa" ? "−" : "+"}{moeda(t.valor)}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border", STATUS_CONFIG_FIN[t.status].cls)}>
                      {STATUS_CONFIG_FIN[t.status].label}
                    </span>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t bg-slate-50/60">
          <span className="flex items-center gap-1.5 text-xs text-slate-400">
            <CreditCard className="w-3.5 h-3.5" />10 de 247 transações
          </span>
          <div className="flex items-center gap-6 text-xs">
            <span className="text-slate-500">Receitas: <span className="font-bold text-emerald-600">{moeda(TOTAL_RECEITAS)}</span></span>
            <span className="text-slate-500">Despesas: <span className="font-bold text-red-500">{moeda(TOTAL_DESPESAS)}</span></span>
